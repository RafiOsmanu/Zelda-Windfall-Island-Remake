...
void ModelComponent::Initialize(const SceneContext& sceneContext)
{
	...
	CameraDepthRenderer::Get()->UpdateMeshFilter(sceneContext, m_pMeshFilter);

	m_enableDepthMapDraw = true;
...
void ModelComponent::DepthMapDraw(const SceneContext& sceneContext)
{
	//TODO: implement DepthMapDraw
	if (m_pMeshFilter->HasAnimations())
		CameraDepthRenderer::Get()->DrawMesh(sceneContext, m_pMeshFilter, GetTransform()->GetWorld(), m_pAnimator->GetBoneTransforms());
	else
		CameraDepthRenderer::Get()->DrawMesh(sceneContext, m_pMeshFilter, GetTransform()->GetWorld());
}
