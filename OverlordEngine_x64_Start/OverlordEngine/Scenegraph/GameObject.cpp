...
void GameObject::RootDepthMapDraw(const SceneContext& sceneContext) const
{
	//makes sure every object is drawn in the depth map
	//--------------------------------------------------
	
	//Component Depth-Draw
	for (BaseComponent* pComp : m_pComponents)
	{
		if (pComp->m_enableDepthMapDraw)
			pComp->DepthMapDraw(sceneContext);
	}

	//Root-Object Depth-Draw
	for (GameObject* pChild : m_pChildren)
	{
		pChild->RootDepthMapDraw(sceneContext);
	}
}
