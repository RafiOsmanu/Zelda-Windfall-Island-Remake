...
#pragma region DEPTH PASS
	//DEPTH_PASS
	//+++++++++++
	TODO_W8(L"Implement Shadow Pass")
	//1. BEGIN > CameraDepthRenderer::Begin (Initiate the DepthPass)
	CameraDepthRenderer::Get()->Begin(m_SceneContext);

	//2. DRAW_LOOP > For every GameObject (m_pChildren), call GameObject::RootDepthMapDraw
	for (const auto pChild : m_pChildren)
	{
		pChild->RootDepthMapDraw(m_SceneContext);
	}

	//3. END > CameraDepthRenderer::End (Terminate the ShadowPass)
	CameraDepthRenderer::Get()->End(m_SceneContext);
...
#pragma region POST-PROCESSING PASS
	...
	if (m_PostProcessingMaterials.size() > 0)
	{
		...
		auto initRt = m_pGame->GetRenderTarget();
		auto prevRt = initRt;
		...
		for (const auto& ppMaterial : m_PostProcessingMaterials)
		{
			if(!ppMaterial->IsEnabled())
				continue;

			ppMaterial->Draw(m_SceneContext, prevRt);

			prevRt = ppMaterial->GetOutput();
		}
		...
		if (initRt != prevRt)
		{
			m_pGame->SetRenderTarget(initRt);
			SpriteRenderer::Get()->DrawImmediate(m_SceneContext.d3dContext, prevRt->GetColorShaderResourceView(), DirectX::XMFLOAT2(0, 0));
		}
